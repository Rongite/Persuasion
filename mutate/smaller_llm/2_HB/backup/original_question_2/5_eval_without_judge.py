...
'''
all_nutations_df = pd.read_csv('/home/jlong1/Downloads/persuasion/Data_n_Code_persuasion/jikailoong/2_persuasion_prompts/2_HB/2_Alliance_Building/2_single_mutations_with_gpt4o_judgements_best_strategies_extract_HP_FT.csv')
...
modify # every 6 questions
'''
checkpoint_file = '/home/jlong1/Downloads/persuasion/Data_n_Code_persuasion/jikailoong/3_evaluation_results/2_HB/2_Alliance_Building/2_eval_results_without_judge.csv'
...
# Function to load checkpoint data and remove already processed rows from all_nutations_df (Auto-resume)
def load_checkpoint():
    if os.path.exists(checkpoint_file):
        completed_df = pd.read_csv(checkpoint_file)
        completed_rows = completed_df[['strategy', 'original_text', 'mutated_version']]
        return all_nutations_df.merge(completed_rows, how='outer', indicator=True).query('_merge == "left_only"').drop(columns='_merge')
    else:
        return all_nutations_df
...
def evaluate_mutations_from_valid_csv(models):
    ...
    all_nutations_df_filtered = load_checkpoint()  # Auto-resume: load checkpoint

    total_rows = len(all_nutations_df_filtered)
    with tqdm(total=total_rows, desc="Processing rows") as pbar:
        for idx, row in all_nutations_df_filtered.iterrows():
...
modify
'''
print("Evaluation complete. Results saved to '2_eval_results_without_judge.csv'.")

'''
modify
modify # every 6 questions
'''
df = pd.read_csv('/home/jlong1/Downloads/persuasion/Data_n_Code_persuasion/jikailoong/3_evaluation_results/2_HB/2_Alliance_Building/2_eval_results_without_judge.csv')
